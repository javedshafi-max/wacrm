...
async function loadAllNodes(
  ...
  // PostgREST caps a response at db-max-rows (1000 by default). A plain
  // .select() therefore SILENTLY returns only the first 1000 nodes of a
  // larger flow — every lookup beyond that returns undefined and the run
  // dies with "node_not_found" on its first hop.
  //
  // This hit production: the 4-branch flow has 1577 nodes, `root_p1` fell
  // outside the first page, and every customer got silence. The previous
  // 522-node flow was under the cap, which is why it never showed up.
  // Page explicitly rather than trusting the default.
  ...
  for (let from = 0; ; from += PAGE) {
    ...
    const rows = (data ?? []) as FlowNodeRow[];
